...
extension SignalProducerType where Value == Moya.Response, Error == Moya.Error {
    ...
    public func mapObject<T: Mappable>(type: T.Type, keyPath: String = "") -> SignalProducer<T, Error> {
    ...
    /// protocol.
    /// If the conversion fails, the signal errors.
    public func mapArray<T: Mappable>(type: T.Type, keyPath: String = "") -> SignalProducer<[T], Error> {
        return producer.flatMap(.Latest) { response -> SignalProducer<[T], Error> in
